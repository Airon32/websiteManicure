...
app.post('/api/client/login', rateLimit({
    ...
    if (whatsAppOtpSender) {
        return res.status(409).json({
            code: 'OTP_REQUIRED',
            error: 'Utilize o envio de código por WhatsApp para entrar na sua conta.'
        });
    }
...
app.post('/api/appointments/:id/cancel', async (req, res) => {
    ...
    if (ownership.data.status === 'concluído') return res.status(409).json({ error: 'Um atendimento concluído não pode ser cancelado.' });
