...
function ConfirmAppointment() {
  ...
  const handleConfirm = async () => {
    ...
    setErrorMessage('');
    try {
      await api.post(`/api/appointments/${id}/confirm`, { token: searchParams.get('token') || '' });
      setStatus('success');
    } catch (err) {
      ...
      <div className="glass-card max-w-md w-full p-8 text-center fade-in-up duration-500 border border-primary/20 shadow-2xl shadow-primary/10">
        ...
        {status === 'error' && (
          <div className="flex flex-col items-center scale-in-center">
            ...
            <p className="text-muted mb-6">{errorMessage || 'O link pode ter expirado ou o agendamento já foi alterado.'}</p>
