  ...
  return (
    ...
    >
      <div className="flex h-full min-w-0 flex-col justify-between gap-0.5">
        ...
        {/* Footer actions when card height allows (e.g. >= 58px) */}
        {height >= 56 && (
...
export default function AgendaTimeline({
  ...
  const moveDate = direction => setSelectedDate(addDays(selectedDate, direction));
  ...
  return (
    <section
      ...
      aria-label="Agenda"
    >
      {/* 1. Ultra-compact Header & Date Navigation Strip */}
      <div className="relative z-30 shrink-0 border-b border-border/50 bg-card/80 backdrop-blur-xl px-2 py-1.5 md:px-4 md:py-2">
        <div className="flex items-center justify-between gap-2 mb-1.5">
          {/* Stepper: Month and Day Navigation */}
          ...
          {/* Quick "Hoje" button & Total stats */}
          <div className="flex items-center gap-2">
            {!selectedDayIsToday && (
              <button
                type="button"
                onClick={() => setSelectedDate(startOfToday())}
                className="h-7 rounded-lg border border-primary/30 bg-primary/15 px-2.5 text-[10px] font-black uppercase text-primary hover:bg-primary/25 transition-colors shadow-sm"
              >
                Hoje
              </button>
            )}
      ...
      {/* 2. Grid Container: Sticky Column Headers + 2D Timeline Grid */}
      <div
        ref={gridScrollContainerRef}
        className="relative flex-1 overflow-auto no-scrollbar flex flex-col"
      >
        <div className={`flex flex-col ${isTwoProfessionals ? 'w-full' : 'min-w-max'}`}>
          ...
          <div className="sticky top-0 z-40 flex border-b border-border/50 bg-card/95 backdrop-blur-2xl">
            ...
            <div
              className={`flex-1 ${
                isTwoProfessionals
                  ? 'grid grid-cols-2'
                  : 'flex min-w-max md:grid md:grid-flow-col md:auto-cols-fr'
              }`}
            >
                ...
                return (
                  <div
                    ...
                    className={`border-r border-border/50 bg-card/50 ${
                      isTwoProfessionals ? 'min-w-0' : 'min-w-[150px] md:min-w-0'
                    }`}
                  >
                    <ProfessionalHeader
              ...
              {visibleProfessionals.map(professional => {
                ...
                return (
                  <div
                    ...
                    className={`relative border-r border-border/40 bg-grid-pattern ${
                      isTwoProfessionals ? 'min-w-0' : 'min-w-[150px] md:min-w-0'
                    }`}
